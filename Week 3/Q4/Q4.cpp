...
typedef map<string, int> GotoMap; // maps productions to LR(0) item ids
...
void printParseTable(vector<LR0Item>& lr0items) {

    for (int i=0; i < lr0items.size(); i++) {
        for (auto go : lr0items[i].gotos) {
            if (isupper(go.first)) {
    ...
    for (int i=0; i < lr0items.size(); i++) {
        for (int j=0; j < lr0items[i].Size(); j++) {
            string rhs = lr0items[i][j]->rhs;
            if (rhs.back() == '@') {
